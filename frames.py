...
class Frames:
    ...
    def __init__(self, mainObj, MainWin, wWidth, wHeight, function, Object, xAxis=10, yAxis=10):
        ...
        self.resLabel = tkinter.Label(self.winFrame, text="", height=1, width=20)
    ...
    def NextWindow(self, methodToExecute):
        ...
        if (current == len(listWF) - 1):
            ...
            self.resLabel['text'] = 'Extraction complete'
            self.resLabel.configure(background="White", font=("Comic Sans MS", 16, "bold"), fg="red")
            self.resLabel.place(x=700, y=400)
        else:
